...
$sql1 = mq("select * from modelposttest where idx='".$idx."'");
...
// 
 if(!isset($_FILES[$field_name])) {
    ...
    if(password_verify($pwk, $bpw)){
        ...
        //boardposttest테이블
		$sql = mq("update modelposttest set id='".$username."',title='".$title."',content='".$content."',date='".$date."',file='".$name."' where idx='".$idx."'");
    ...
    if ($result === false) { // false가 나왔다면 무슨 에러인지 출력한다(29번 줄의  태그를 주석 쳐야 제대로 볼 수 있다)
        echo mysqli_error($db);
         ...
         echo "<script>
         alert('글수정이 완료되었습니다.');
         location.href='model.php';</script>";
         echo $userid;
         echo $userpw;
         echo $title;
         echo $content;
...
<script type="text/javascript">alert('수정되었습니다.'); location.replace("modelviewreplytest.php?idx=<?php echo $idx; ?>");</script>
<?php
