...
// 
 if(!isset($_FILES[$field_name])) {
    ...
    if($title !='' && $subtitle !='' && $price !='' && $description !=''){
     ...
     $sql="insert into shopdatapopular(category, title, subtitle, price, description, img, size, size1, size2, color, color1, color2) values('$category','$title','$subtitle','$price','$description','$name','$size1','$size2','$size3','$color','$color1','$color2');";    
     $result = mysqli_query($db, $sql);
         ...
         echo "<script>
         alert('상품등록이 완료되었습니다.');
         location.href='shop_list_popular.php';</script>";
         echo $title;
         echo $subtitle;
         echo $price;
         echo $description;
