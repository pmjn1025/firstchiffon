...
// if문으로, 파일폼에 값이 없을경우 update문을 실행하지 않게하여 구현
...
<script type="text/javascript">alert('수정되었습니다.'); location.replace("viewpagereply.php?idx=<?php echo $idx; ?>&&idxpost=<?php echo $idxpost;?>&&category=<?php echo $category;?>");</script>
<?php
		 ...
		 if(!isset($_FILES[$field_name])) {
			...
			if ($result === false) { // false가 나왔다면 무슨 에러인지 출력한다(29번 줄의  태그를 주석 쳐야 제대로 볼 수 있다)
				echo mysqli_error($db);
				 ...
				 echo "<script>
				 alert('수정되었습니다.');
				 location.href='viewpagereply.php?idx=$idxpost';</script>";
				 echo $userid;
				 echo $userpw;
				 echo $title;
				 echo $content;
