...
<script type="text/javascript">alert('수정되었습니다.'); location.replace("viewpagereplytop.php?idx=<?php echo $idx; ?>&&idxpost=<?php echo $idxpost;?>&&category=<?php echo $category;?>");</script>
<?php
		 ...
		 if(!isset($_FILES[$field_name])) {
			...
			if ($result === false) { // false가 나왔다면 무슨 에러인지 출력한다(29번 줄의  태그를 주석 쳐야 제대로 볼 수 있다)
				echo mysqli_error($db);
				 ...
				 echo "<script>
				 alert('수정 되었습니다.');
				 location.href='viewpagereplytop.php?idx=$idxpost';</script>";
				 echo $userid;
				 echo $userpw;
				 echo $title;
				 echo $content;
