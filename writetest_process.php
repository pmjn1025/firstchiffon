...
// 
 if(!isset($_FILES[$field_name])) {
    ...
    if($username && $userpw && $title && $content){
        ...
        //boardposttest테이블
        $sql = "INSERT INTO boardposttest(id, pw, title, content, date, lock_post,file) VALUES('$username', '$userpw', '$title', '$content', '$date', '$lo_post', '$name');"; 
    ...
    if ($result === false) { // false가 나왔다면 무슨 에러인지 출력한다(29번 줄의  태그를 주석 쳐야 제대로 볼 수 있다)
        echo mysqli_error($db);
         ...
         echo "<script>
         alert('글쓰기 완료되었습니다.');
         location.href='contacttestlogin.php';</script>";
         echo $userid;
         echo $userpw;
         echo $title;
         echo $content;
