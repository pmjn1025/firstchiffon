...
$sql1 = mq("select * from shopdatatop where idx='".$idx."'");
// 따라서 해당 컬럼(배열로 되어있는 값)을  $board로 변수로 받는다.
$board = $sql1->fetch_array();


if($title !='' && $price !='' && $description !=''){
    $sql = mq("update shopdatatop set category='".$category."',title='".$title."',description='".$description."',price='".$price."',size='".$size1."',size1='".$size2."',size2='".$size3."',color='".$color."',color1='".$color1."',color2='".$color2."' where idx='".$idx."'");
?>

<script type="text/javascript">alert('수정되었습니다.'); location.replace("shop_list_top.php?idx=<?php echo $idx; ?>");</script>
<?php
