...
$sql = mq("select * from blackpantsposttest where idx='".$bno."'"); /* 받아온 idx값을 선택 */
...
<?php
	 	...
	 	if(isset($_POST['pw_chk'])) //만약 pw_chk POST값이 있다면
	 	{
			...
			?>
<script type="text/javascript">
location.replace("blackpantsviewreplytest.php?idx=<?php echo $board["idx"]; ?>");
</script><!-- pwk와 bpw값이 같으면 read.php로 보내고 -->
<?php 
			}else{ ?>
<script type="text/javascript">
location.replace("blackpants.php");
alert('비밀번호가 틀립니다');
</script>
