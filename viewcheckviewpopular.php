	 ...
	 <?php
	 	...
	 	if(isset($_POST['pw_chk'])) //만약 pw_chk POST값이 있다면
	 	{
			...
			if(password_verify($pwk,$bpw)) //다시 if문으로 DB의 pw와 입력하여 받아온 bpw와 값이 같은지 비교를 하고
			{
				$pwk == $bpw;
			?>    
				<script type="text/javascript">location.replace("viewpagereplypopular.php?idx=<?php echo $board["idx"]; ?>&&idxpost=<?php echo $idxpost;?>&&category=<?php echo $category;?>");</script><!-- pwk와 bpw값이 같으면 read.php로 보내고 -->
			<?php 
			}else{ ?>
				<script type="text/javascript">location.replace("viewpagepopular.php?idx=<?php echo $idxpost;?>");alert('비밀번호가 틀립니다');</script><!--- 아니면 비밀번호가 틀리다는 메시지를 보여줍니다 -->
			<?php } } ?>