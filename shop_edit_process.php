...
$sql1 = mq("select * from shopdata where idx='".$idx."'");
...
die("업로드디렉토리생성에실패했습니다.");
 ...
 if(!isset($_FILES[$field_name])) {
    ...
    if($pname !='' && $comment !='' && $price !='' && $memo !=''){
        ...
        $sql = mq("update shopdata set name='".$pname."',comment='".$comment."',memo='".$memo."',price='".$price."',img='".$name."' where idx='".$idx."'");
...
<script type="text/javascript">alert('수정되었습니다.'); location.replace("shop_list.php?idx=<?php echo $idx; ?>");</script>
<?php
